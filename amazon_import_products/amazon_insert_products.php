				...
				while(($content = fgets($file)) !== FALSE)									// Reading file line by line
				{
					if($a != '1')															// Leaving header line from csv file
					{
						...
						if($content_arr[41] == '1' || $content_arr[41] == '0')				// Only getting Shiping Detail products
						{
							...
							echo "\n offerprice_open :".$content_offerprice." \n"."listprice_open : ".$content_listprice." \n lowest_new_price".$content_lowestnewprice." \n"; 
							if($content_offerprice == '' || $content_offerprice == 1)
							{	
								echo "\n Offerprice blank".$content_offerprice." \n";
								if($content_listprice == '')
								{
									echo "\n Listprice blank".$content_listprice." \n"; 
									echo "\n LowestNewPrice".$content_lowestnewprice." \n";
									$prod_price = preg_replace('/\s+/', ' ', trim($content_lowestnewprice));
								}
								else
								{
									echo "\n Listprice :".$content_listprice." \n";
									$prod_price = preg_replace('/\s+/', ' ', trim($content_listprice));
								} 																	
							}
							else{
								echo "\n Offerprice :".$content_offerprice." \n";
							...
							if($prod_price >= '50' && $prod_price <= '2500')
							{
								if(trim($content_itemweight) != '' || trim($content_itemweight) <= 0)
								{
									...
									if($prod_qty > 0)
									{
										...
										{
										//	$prod_weight 		= preg_replace('/\s+/', ' ', trim($content_itemweight));	// Weight from ItemDimensions
										if($content_weight == ''){$prod_weight 		= preg_replace('/\s+/', ' ', trim($content_itemweight));}
										else{$prod_weight 		= preg_replace('/\s+/', ' ', trim($content_weight));}
											//$prod_weight 		= preg_replace('/\s+/', ' ', trim($content_weight));  		// Weight from PackageDimensions edite on 17-DEC-014
											...
											if($prod_price <= '2500')
											 {
												...
												$amazon_raw_img		= $base_url_magento.'amazon_import_products/liki_img/cache/'.$prod_img_name.'_'.$price.'_'.$prod_sku.'.'.									                                                $prod_img_ext;                                                      // Path of Amazon raw image 
