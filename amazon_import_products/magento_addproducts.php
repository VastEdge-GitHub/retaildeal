		...
		foreach($files_process as $file)
		{
			...
			if(is_file($file))
			{
				if($new_mainfile == 1)
				{
					...
					fwrite($main_fh,'^store^|^websites^|^attribute_set^|^type^|^category_ids^|^sku^|^name^|^image^|^amazon_raw_img^|^small_image^|^thumbnail^|^amazon_prime^|^liki_desription^|^vastedge_meta_robots^|^price^|^weight^|^standard_shipping^|^status^|^visibility^|^tax_class_id^|^wp_amazon_sync^|^wp_amazon_use_categories^|^feature^|^description^|^short_description^|^wp_amazon_local^|^wp_amazon_asin^|^wp_amazon_ean^|^wp_amazon_offer_condition^|^wp_amazon_offer_price_type^|^wp_amazon_offer_price^|^wp_amazon_offer_currency^|^wp_amazon_offers_list_url^|^wp_amazon_product_url^|^wp_amazon_reviews_url^|^liki_price^|^monthly_price^|^qty^|^min_qty^|^use_config_min_qty^|^is_qty_decimal^|^backorders^|^min_sale_qty^|^use_config_min_sale_qty^|^max_sale_qty^|^use_config_max_sale_qty^|^is_in_stock^|^use_config_manage_stock^|^product_name^|^store_id^|^product_type_id^|^sales_rank^|^manufacturer^|^meta_description^|^meta_keyword^|^free_shipping^|^featured^'.PHP_EOL);
					 
				fclose($main_fh); 
			    	}
	/*RD-234 change for add amazon raw image : amazon_raw_img ,monthly_price */			
